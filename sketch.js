...
function draw() {
  ...
  for (var i = 0; i < balls.length; i++) {
    ...
    for(var g = 0;g<boats.length;g++){
      if( balls [i]!= undefined && boats[g]!= undefined){
        var collsion = Matter.SAT.collides(balls[i].body,boats[g].body)
...
function showBoats() {
  if (boats.length > 0) {
    ...
    for (var i = 0; i < boats.length; i++) {
      ...
      boats[i].display();
      boats[i].animate();
     var collsion = Matter.SAT.collides(tower.body,boats[i].body)
